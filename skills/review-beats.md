...
- 若出现其他地点名称（如当前场景是"宇航局主监控室"，但 beat 中描述了"空间站窗外"或"龙尸横在太空"），标记为"节拍地点与场景地点冲突，违反空间绑定原则"。
- 检查是否使用"与此同时""时间跳转"等跨空间转场词，若出现，标记为"跨空间描述，应拆分为新场景"。

### 6. 重复内容检查（新增）
- 若当前场景的节拍与已生成的其他场景存在明显的画面或反应重复（如同一句"那是……"、同一段龙尸描述），标记为"节拍级重复，应删除或提供信息增量"。
- 修正指引：若发现重复，明确告知哪个场景已呈现过该内容，并要求当前场景删除重复部分或改写为独有视角。
